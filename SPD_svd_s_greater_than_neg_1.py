...
# SVD
from util import *
def SPD_svd_s_greater_than_neg_1(k, n):
    np.random.seed(1)
    ...
    Z=Circle(X,Y)
    plt.contour(X,Y,Z,1)
    plt.savefig('SPD_svd_s_greater_than_neg_1.png')
    return xsshaped

def evaluate_SPD_svd_s_greater_than_neg_1(k, n):
    ...
    for i in range(random_iterations_number):
        ...
        x = SPD_svd_s_greater_than_neg_1(k, n)
        end_time = time.time()
...
if __name__ == '__main__':
    n = 2
    k = 3
    SPD_svd_s_greater_than_neg_1(k, n)    